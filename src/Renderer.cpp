...
namespace lava
{
	Renderer::Renderer(Window* _window, const std::vector< std::string >& _requiredExtensions,
		const std::vector< std::string >& _requiredLayers,
		std::function<void(VkCommandBuffer)> _renderingFnc)
		: mWindow(_window)
		, mDebug(nullptr)
		, mRenderingFnc(_renderingFnc)
	{
		...
		{
			if (strcmp(l.layerName, "VK_LAYER_NV_nsight") == 0)
			{
				layers.push_back("VK_LAYER_NV_nsight");
			}
			else if (strcmp(l.layerName, "VK_LAYER_LUNARG_standard_validation") == 0)
			{
	...
	void Renderer::Update()
	{
		...
		mRenderingFnc(mCommandBuffers[mCurrentBackBuffer]);
	...
	const uint32_t Renderer::GetFrameBufferWidth() const
	{
		return mWindow->GetWidth();
	}

	const uint32_t Renderer::GetFrameBufferHeight() const
	{
